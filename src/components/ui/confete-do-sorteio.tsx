 ...
 * quantos times o fut é antes de a pessoa ler qualquer coisa. Time sem colete
 * (cor nula) não entra na chuva: confete numa cor de reserva mentiria sobre o
 * que está na tela. Na confirmação de presença fica só a bola: confete na
 * interação mais repetida do app cansa em uma semana.
 *
 ...
 */
export function ConfeteDoSorteio({
  chave,
  times,
}: {
  chave: string;
  times: { cor: string | null }[];
}) {
  const canvasRef = useRef<HTMLCanvasElement>(null);

  // As cores viram UMA string antes de chegar ao efeito, e o efeito depende
  // dela e não do array. `times` vem do payload RSC (`teamList`), então toda
  // atualização entrega um array novo com o mesmo conteúdo — e como a
  // dependência mudava, o efeito se desmontava, chamava `reset()` e matava a
  // chuva no ar. Bastava confirmar presença nos ~3s do confete para ele sumir
  // e não voltar, porque a marca do sessionStorage faz a re-execução desistir.
  // String igual, dependência igual, chuva intacta.
  const cores = times
    .map((t) => t.cor)
  ...
  useEffect(() => {
    ...
    const lista = cores.split("|").filter(Boolean);
    ...
    void import("canvas-confetti").then(({ default: confetti }) => {
      ...
      void disparar({ particleCount: 80, spread: 72, startVelocity: 38, origin: { y: 0.3 }, colors: lista });
    });
